...
# Adds neighbors to queue, dfs will favor neighbors from left-to-right.
def dfs_path(Node, queue, visited):
    visited.add(Node)
    try:
        queue.extendleft(reversed([x for x in prioritize(Node) if x not in visited and x not in queue]))
    except TypeError:
        pass
"""
To do: Random walks
